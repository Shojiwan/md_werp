...
path         <- 'C:/001_MD_WERP/001_Culgoa/002_TFV/culgoa_614/'
pth2         <- 'C:/001_MD_WERP/001_Culgoa/002_TFV/culgoa_665/'
...
veg_type@crs <- crs(mask@proj4string)
veg_type <- crop(veg_type, y = mask@bbox)
# ----

# APPLY CHANGES TO WIND HERE ----
# This is wind speed at 1m based on wind speed at 10m and reductions
wind$u.e <- wind$u_kph; wind$v.e <- wind$v_kph
wind$u.f <- 4 * (wind$red_F * wind$u_kph); wind$v.f <- 4 * (wind$red_F * wind$v_kph)
wind$u.a <- 4 * (wind$red_A * wind$u_kph); wind$v.a <- 4 * (wind$red_A * wind$v_kph)
wind$u.b <- 4 * (wind$red_B * wind$u_kph); wind$v.b <- 4 * (wind$red_B * wind$v_kph)
# No WRF application
...
for (month in model_months) {
  myr  <- ifelse(month %in% 1 : 4, model_year + 1, model_year) 
  tmpW <- wind[which(month(wind$date) == month & year(wind$date) == myr), ]
  ...
  saveRDS(object = wind_rstr,
          file = paste0(path, '002_TFV/in/wind_',
                        ...
                        '.RData'))
  # Create NDCF
  ncFl <- paste0(pth2, '002_TFV/in/wind_',
                 ifelse(data[['month']]$numr[month] < 10, '0', ''),
                 data[['month']]$numr[month], '_', data[['month']]$mnth[month],
                 '.nc')
